...
knitr::opts_chunk$set(echo = FALSE,fig.width = 9,fig.height = 6,message = FALSE,warning = FALSE)
library(here)
library(ggplot2)
library(dplyr)
library(tidyr)
...
for (j in 1:length(All_Models)) 
{
  ...
  Final_Data<-Final_Data_Non_Identical_r0
  ...
  Final_Data[(Final_Data$Model %in% Selected_All) & (Final_Data$Metric == "MSE"),] %>%
    ...
    geom_point(size=3.75,shape=18)+geom_line(size=1,linetype=6)+
    scale_color_manual(values=Colors_All)+
    theme_light()+xlab("Sub sample size")+
    scale_x_continuous(breaks = unique(Final_Data$Subsample_Size))+
    ggtitle("Simulated MSE")+
    ylab("Estimated Values")->p1
