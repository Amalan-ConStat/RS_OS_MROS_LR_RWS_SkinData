---
title: "Rate Event Random_Sampling"
author: "M.Amalan"
...
for (j in 1:length(All_Models)) 
{
  ...
  load(here("Non_Identical_r0","Outputs","Classical","RE_Random_Sampling",
            paste0("RE_Random_Sample_output_",j,".RData")))
  
  Est_Parameter<-as.data.frame(Est_Param_RE_RandomSample)
  colnames(Est_Parameter)<-c("Subsample_Size",paste0("Theta",0:length(All_Models[[j]][-1])))
  
  Utility<-as.data.frame(Utility_RE_RandomSample) 
  colnames(Utility)<-c("Subsample_Size",paste0(c("A","D"),"_optimality"))
  
  Est_Bias<-as.data.frame(Bias_RE_RandomSample) 
  colnames(Est_Bias)<-c("Subsample_Size",paste0("Theta",0:length(All_Models[[j]][-1])))
  
  remove(Est_Param_RE_RandomSample,Utility_RE_RandomSample,Bias_RE_RandomSample)
