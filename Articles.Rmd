...
Theme_special_1<-function()
{
  theme(axis.text.x = element_text(color = "black",size=18), #, vjust = 0.5
        axis.text.y = element_text(color = "black",size=16), #, vjust = 0.5, hjust=1
...
Combined_MSE %>%
  filter(!(Method %in% c("Optimal Sub-sampling mVc","Model Robust Optimal Sub-sampling mVc"))) %>%
  group_by(Metric,Method,Subsample_Size) %>%
  summarise(Sum_MSE=sum(`Estimated Values`)) %>%
  ggplot(.,aes(x=Subsample_Size,y=log(Sum_MSE),color=Method))+
  scale_x_continuous(labels =unique(Combined_MSE$Subsample_Size),
    ...
    ylab("log(Summed SMSE)")+expand_limits(y = 0)+
    scale_y_continuous(breaks = scales::pretty_breaks(n = length(unique(Combined_MSE$Subsample_Size))/2))+
    guides(colour = guide_legend(nrow = 1))+Theme_special_1()

ggsave(filename = "Poster_RWA_LR_SkinData.eps",device="eps",dpi=300,width = 6,height = 8,units = "in")
